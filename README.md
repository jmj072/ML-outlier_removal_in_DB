...
- ECG 심박수 데이터를 표준화한 후, Z-value > 2인 값은 **이상치**로 판단하여 **ECG 심박수의 평균값**으로 변환
	<img src = "https://user-images.githubusercontent.com/77204538/177088625-c7088b1b-db82-49e2-9c0d-d5a002e5461b.png"  width=650 height=200>

### 2.2 레이더로 측정된 심박수 데이터 전처리
- 측정된 심박수 데이터를 표준화 한 후, Z-value > 2인 값은 **이상치**로 판단하여 **측정값의 평균**으로 변환
	<img src = "https://user-images.githubusercontent.com/77204538/177088676-992284c1-fccd-4489-ab00-5319db267d01.png"  width=650 height=300>
...
> ☑ 오류 데이터를 제거한 이후, 예측 모델의 미세한 성능 향상을 확인하였음
...
- 오류 데이터 제거 이후, 일부 데이터셋에서는 미세한 성능 향상, 성능 감소가 확인됨
	...
	- 오류 데이터를 제거했음에도, 여전히 오류율 2% 이상의 데이터가 확인됨
		...
		- Dataset이 시계열 데이터였기 때문에 HoldOut 방법을 사용하였지만, 오류 데이터가 랜덤으로 validation set에 포함되는 점을 고려하지 못하였음
	- EDA 과정에서 일부 특성들간의 연관관계(상관계수 0.5 이상)가 확인 ⇒ 다중공선성에 의해 회귀모델의 성능이 저하된 것으로 예상됨

<br> 

### 개선사항
- Kfold  교차검증과 같은 전체 데이터셋을 살펴볼 수 있는 방법으로 모델 학습을 진행한다면, 개선될 가능성이 있음
	- 시계열 데이터이지만, 데이터 전처리 과정 중 BPM 단위로 된 심박수를 정적 데이터(RR interval)로 변환하는 과정을 거침
	
		<img src = "https://user-images.githubusercontent.com/77204538/177091406-b162de7a-592b-4f9d-bce3-1c14d7e3cf8e.png" width=450 height=250>
	
	- 따라서 Kfold 사용시 `shuffle=True`로 하여 오류율이 1% 이상인 데이터를 따로 분리한다면, 오류 데이터를 골고루 살펴볼 수 있을 것이라 생각됨
	
- 연관되어있는 특성들은 PCA로 전처리 한후, 모델 학습을 진행하면 개선될 수 있음
	- 연관되어 있는 특성들은 정규분포를 가지고 있지 않을 가능성이 있으므로, 확인 후 정규분포를 가진다면 PCA, 가지고 있지않다면 kernel PCA를 수행해야함
